...
def _unwrap_manager_queryset(instance: Instance) -> Instance | None:
    """Return the given QuerySet type as-is, or a Manager's queryset held as its second type arg."""
    if not instance.type.has_base(fullnames.BASE_MANAGER_CLASS_FULLNAME):
        return instance
    qs_arg = get_proper_type(instance.args[1]) if len(instance.args) > 1 else None
