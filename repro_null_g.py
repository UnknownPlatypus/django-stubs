"""Option G: type_check_only metaclass with __call__ overloads.

Idea: declare a metaclass on Field whose __call__ overloads bind _NT for any
subclass. Type checkers consult the metaclass __call__ when type-checking
SubClass(...). Single declaration site, no per-subclass overloads.
"""
...
_FieldSelf = TypeVar("_FieldSelf", bound="Field[Any, Any, Any]")
...
class _FieldMeta(type):
    @overload
    def __call__(
        cls: type[Field[_ST, _GT, _NT]], *, null: Literal[False] = False, **kwargs: Any
    ) -> Field[_ST, _GT, Literal[False]]: ...
    @overload
    def __call__(
        cls: type[Field[_ST, _GT, _NT]], *, null: Literal[True], **kwargs: Any
    ) -> Field[_ST, _GT, Literal[True]]: ...


class Field(Generic[_ST, _GT, _NT], metaclass=_FieldMeta):
